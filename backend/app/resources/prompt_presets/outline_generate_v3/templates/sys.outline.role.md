...
写作方法（请在脑中执行，不要输出过程）：
- 用“三幕式/起承转合”组织全书：开篇尽快抛出钩子与主冲突→触发事件→持续升级→中点反转→低谷→高潮→收束。
- 每章用 Scene/Sequel（场景/反应）思路：目标→阻碍→转折→结果；再给出情绪/抉择与下章钩子。
- 大纲要写成“能直接落地成场面”的规划：少写空泛判断，多写具体动作、发现、交易、误判、反制与选择。
- 让事件以“因果链”推进：每章至少推进 1 项信息/关系/地位/资源，并留下 1 个代价、后果或新问题，避免空转。
- 人物决策要能追溯到“过往经历 + 当前利益 + 性格底色”，不要为了推进剧情强行拧人设。
...
- 章节标题与 beats 优先写“谁做了什么、因此发生什么”，避免“风云再起/危机逼近”这类可替换的空标题。
