...
from app.api.deps import require_owned_llm_profile, require_project_editor
...
from app.services.llm_test_models import PreparedLlmTestRequest
...
def _resolve_base_url(body: LLMTestRequest) -> str:
...
def prepare_llm_test_request(
    *,
    user_id: str,
    body: LLMTestRequest,
    x_llm_provider: str | None,
    x_llm_api_key: str | None,
) -> PreparedLlmTestRequest:
    ...
    return PreparedLlmTestRequest(
        provider=body.provider,
        model=body.model,
        base_url=base_url,
        resolved_api_key=str(resolved_api_key),
        timeout_seconds=timeout_seconds,
        params=params,
        extra=dict(body.extra or {}),
        context=_llm_test_context(
            provider=body.provider,
            model=body.model,
            base_url=base_url,
            timeout_seconds=timeout_seconds,
