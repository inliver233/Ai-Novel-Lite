## Task: MC-003 - Rewrite PromptsVectorRagSection with tab toggle

You must COMPLETELY REWRITE the file `frontend/src/pages/prompts/PromptsVectorRagSection.tsx`.
The old implementation has Embedding and Rerank in separate `<details>` sections with lots of "effective" status displays.
The new design puts Embed/Rerank in ONE panel with a tab toggle, shared form fields, and a collapsible params section.

### Read these files first
1. `frontend/src/pages/prompts/PromptsVectorRagSection.tsx` (current - will be replaced)
2. `frontend/src/pages/prompts/models.ts` (VectorRagForm type + defaults)
3. `frontend/src/types.ts` (ProjectSettings type)

### KEY DESIGN CHANGES
1. Tab toggle at top-left: [Embedding] [Rerank] - switches which form fields are shown
2. Save + Test buttons at top-right
3. Three shared-layout fields: API URL / API Key / Model Name (different data per tab)
4. Collapsible "参数设置" section with different content per tab
5. REMOVE all the "当前有效" effective status displays (too verbose)
6. REMOVE the "当前生效" info block at the top
7. REMOVE the dry-run results display section (keep the test buttons which trigger dry-runs)

### CRITICAL: Keep the same Props type and export name

The file exports `PromptsVectorRagSectionProps` type and `PromptsVectorRagSection` function.
Keep the EXACT SAME props interface - the parent component already passes these props.
But we need to ADD a `ragActiveTab` and `setRagActiveTab` prop. Since we can't change the parent yet (that's MC-006), use internal state for the tab for now:
...
export function PromptsVectorRagSection(props: PromptsVectorRagSectionProps) {
  ...
  return (
    <section className="panel p-6" id="rag-config">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={savingVector || !anyDirty}
            onClick={onSave}
            type="button"
          >
            保存
      ...
      {/* Dirty hint */}
      {anyDirty ? (
        <div className="mt-2 text-[11px] text-warning">有未保存的更改，请先保存再测试。</div>
      ) : null}
            ...
            <>
              ...
              <label className="grid gap-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setVectorForm((v) => ({ ...v, vector_embedding_base_url: e.target.value }))
                  }
                />
              </label>
              <label className="grid gap-1">
              ...
              <label className="grid gap-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setVectorForm((v) => ({ ...v, vector_embedding_model: e.target.value }))
                  }
                />
              </label>
            </>
            ...
            <>
              ...
              <label className="grid gap-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setVectorForm((v) => ({ ...v, vector_rerank_model: e.target.value }))
                  }
                />
              </label>
            </>
              ...
              <>
                ...
                <label className="grid gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setVectorForm((v) => ({ ...v, vector_embedding_provider: e.target.value }))
                    }
                  >
              ...
              <>
                ...
                <label className="flex items-center gap-2 text-sm text-ink">
                  <input
                    ...
                    onChange={(e) =>
                      setVectorForm((v) => ({ ...v, vector_rerank_enabled: e.target.checked }))
                    }
                    type="checkbox"
                  />
                  启用 rerank
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="grid gap-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setVectorForm((v) => ({ ...v, vector_rerank_method: e.target.value }))
                      }
                    >
                ...
                <label className="grid gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setVectorForm((v) => ({ ...v, vector_rerank_provider: e.target.value }))
                    }
                  >
                    <option value="">（使用后端环境变量）</option>
...
### IMPORTANT NOTES
1. The `PROMPTS_COPY` import and `UI_COPY` import from the old file can be REMOVED
2. Keep the `RequestIdBadge` import for error displays
3. Use double quotes for string attributes (project convention)
4. The tab toggle uses inline button styles, NOT a separate component
5. String literals in onChange handlers should use single quotes inside template literals
6. All the onBlur validation logic for rerank params (top_k, timeout, hybrid_alpha) must be kept exactly as in the old file
