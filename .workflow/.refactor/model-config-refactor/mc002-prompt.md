## Task: MC-002 - Rewrite LlmPresetPanel as unified main config block

You must COMPLETELY REWRITE the file `frontend/src/components/prompts/LlmPresetPanel.tsx`.
The old implementation renders 6 separate card components (ConnectionCard, ModelSelectorCard, ParameterTunerCard, ThinkingConfigCard, AdvancedConfigCard). The new design consolidates everything into ONE panel.

### IMPORTANT: Read these files first
1. `frontend/src/components/prompts/LlmPresetPanel.tsx` (current file - will be completely replaced)
2. `frontend/src/components/prompts/types.ts` (LlmForm type)
3. `frontend/src/components/prompts/cards/SliderInput.tsx` (reuse this)
4. `frontend/src/types.ts` (LLMProvider, LLMProfile types)

### CRITICAL RULES
1. Keep the EXACT SAME exported Props type and function signature - the parent still passes the same props
2. The Props type is lines 16-74 of the current file (TaskModuleView + Props types). Copy them as-is.
3. Remove imports for ConnectionCard, ModelSelectorCard, ParameterTunerCard, ThinkingConfigCard, AdvancedConfigCard
4. Keep import for TaskOverrideSection from "./cards"
5. Keep import for SliderInput from "./cards/SliderInput"
6. Keep import for deriveLlmModuleAccessState from "./llmConnectionState"
7. Use double quotes for JSX string attributes (project convention)

### NEW FILE CONTENT

Write the complete new file. Here is the exact structure:
...
The exported function `LlmPresetPanel` should contain:

**Computed values (before return):**
```typescript
...
**Reasoning control helper function (inside component, before return):**
```typescript
  ...
  if (isGemini) {
    return (
      <SliderInput
        ...
        onChange={(v) =>
          props.setLlmForm((prev) => ({ ...prev, gemini_thinking_budget: v }))
        }
        disabled={sharedSaving}
      />
    );
...
**JSX Return (the complete return statement):**
    ...
    <div className="mt-4 grid gap-3">
      ...
      <label className="grid gap-1">
        ...
        <input
          ...
          onChange={(e) =>
            props.setLlmForm((v) => ({ ...v, base_url: e.currentTarget.value }))
          }
        />
      </label>
      ...
      <label className="grid gap-1">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) =>
              props.setLlmForm((v) => ({ ...v, model: e.currentTarget.value }))
            }
          />
        ...
        <label className="grid gap-1">
          ...
          <input
            ...
            onChange={(e) =>
              props.setLlmForm((prev) => ({
                ...prev,
                timeout_seconds: e.currentTarget.value,
              }))
            }
          />
        </label>
...
### VERIFICATION
After writing the complete file, run:
```bash
